...
import '../models/broiler.dart';

class ParentFlockManagementBroilerViewModel
    extends StateNotifier<BaseState<ParentFlockManagementBroilerState>>
    with BaseViewModel {
  final Tool? tool;
  final Repository? repository;
  ParentFlockManagementBroilerViewModel(this.tool, this.repository)
      : super(BaseState(
            data: ParentFlockManagementBroilerState(
                parameters: repository?.getParentFlockManagementParameters() ??
  ...
  calculateValues() {
    ...
    Expression broilerPerYearExp =
        p.parse(broilerEquations?.broilersPerYear ?? '');
    Expression chickenPlacedExp =
        p.parse(broilerEquations?.chickenPlacedBeforeDeath ?? '');
    ...
    double? broilerPerYears =
        broilerPerYearExp.evaluate(EvaluationType.REAL, cm);
    state.data.broilersPerYear = (broilerPerYears ?? 0).round();

    cm
      ..bindVariable(broilerPerYearVar, Number(broilerPerYears ?? 0))
      ..bindVariable(
          broilerLivabilityVar,
          Number(state.data.parameters?.broilerLivability ??
              tool?.equation?.defaults?.broilerLivability?.defaultValue ??
              0));
    ...
    cm
      ..bindVariable(chickenPlacedBeforeDeathVar, Number(chickenPlaced ?? 0))
      ..bindVariable(
          hatchVar,
          Number(state.data.parameters?.hatch ??
              tool?.equation?.defaults?.hatch?.defaultValue ??
              0));
    double? placedEggs = placedEggsExp.evaluate(EvaluationType.REAL, cm);
    state.data.placedEggs = (placedEggs ?? 0).round();

    cm
      ..bindVariable(placedEggsVar, Number(placedEggs ?? 0))
      ..bindVariable(
          hatchingHenVar,
          Number(state.data.parameters?.hatchingHen ??
              tool?.equation?.defaults?.hatchedHen?.value ??
              0));
    double? placedHens = placedHensExp.evaluate(EvaluationType.REAL, cm);
    state.data.hen = (placedHens ?? 0).round();

    cm
      ..bindVariable(henVar, Number(placedHens ?? 0))
      ..bindVariable(
          pulletLivabilityToCapVar,
          Number(state.data.parameters?.pulletLivability ??
              tool?.equation?.defaults?.pulletLivabilityToCap?.defaultValue ??
              0));
